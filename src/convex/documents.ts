...
interface DocumentWithChildren extends Doc<"documents"> {
...
export const createDocument = mutation({
  ...
  handler: async (ctx, args) => {
    ...
    if (args.parentDocument) {
      ...
      const lastRootDoc = await ctx.db
        .query("documents")
        .withIndex("by_position_user", (q) => 
          q.eq("userId", userId)
        )
        .order("desc")
        .first();
      newPosition = (lastRootDoc?.position ?? 0) + 1;
    }
...
export const getDocumentsByParentId = mutation({
  args: { 
    parentDocument: v.optional(v.id("documents")),
    activeOrgId: v.optional(v.string()),
  },
  handler: async (ctx, args) => {
    ...
    if (args.activeOrgId) {
      const isMember = await hasOrgAccess(ctx, args.activeOrgId);
      if (!isMember) {
        return undefined;
      }
...
export const updateDocumentParent = mutation({
  args: { 
    id: v.id("documents"),
    position: v.optional(v.number()),
    metaData: v.optional(v.object({
  ...
  handler: async (ctx, args) => {
    ...
    const document = await ctx.db.patch(args.id, {
      ...
      position: args.position || existingDocument.position,
    })
