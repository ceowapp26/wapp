...
import { X, ArrowUp, Mic, Paperclip, Image, Video, Music, AlignHorizontalSpaceAround, AlignVerticalSpaceAround, RotateCw, Send, StopCircle, RefreshCw, FileText, FileImage, FileAudio } from 'lucide-react';
...
const ChatView = ({
  ...
  const handleGenerate = useCallback(async() => {
    handleSetup();
    if (_content === '' &&  uploadedFiles.length === 0 ||  _content.trim() === '' && uploadedFiles.length === 0 || useDocumentStore.getState().generating) return;
    const combinedContent = uploadedFiles.length > 0
      ? `${_content}\n\nUploaded Files:\n${uploadedFiles.map(file => `${file.name}: ${file.content}`).join('\n')}`
      : _content;
    console.log("this is combinedContent", combinedContent)
    const updatedChats: ChatInterface[] = JSON.parse(
      JSON.stringify(useDocumentStore.getState().chats)
    );
    const currentChat = updatedChats[currentChatIndex];
  ...
  ), [handleGenerate, stop, isOpen, isVertical, options, setFileFormat]);
