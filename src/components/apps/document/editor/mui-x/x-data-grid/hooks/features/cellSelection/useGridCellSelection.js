...
import * as  _gridCellSelectionSelector from "./gridCellSelectionSelector";
...
export const useGridCellSelection = (apiRef, props) => {
  ...
  const isCellSelected = React.useCallback((id, field) => {
    ...
    const cellSelectionModel = _gridCellSelectionSelector.gridCellSelectionStateSelector(apiRef.current.state);
  ...
  const getCellSelectionModel = React.useCallback(() => {
    return _gridCellSelectionSelector.gridCellSelectionStateSelector(apiRef.current.state);
  }, [apiRef]);
  ...
  const handleMouseUp = _utils.useEventCallback(() => {
  ...
  const handleCellMouseDown = React.useCallback((params, event) => {
    ...
    const document = _utils.ownerDocument(apiRef.current.rootElementRef?.current);
  ...
  const handleCellClick = _utils.useEventCallback((params, event) => {
    const {
      id,
      field
    } = params;
  ...
  const handleCellKeyDown = _utils.useEventCallback((params, event) => {
  ...
  React.useEffect(() => {
    ...
    return () => {
      stopAutoScroll();
      const document = _utils.ownerDocument(rootRef);
