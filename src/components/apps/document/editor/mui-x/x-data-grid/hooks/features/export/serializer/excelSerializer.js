...
import { GRID_DATE_COL_DEF } from '../../../../colDef/gridDateColDef';
import { GRID_DATETIME_COL_DEF } from '../../../../colDef/gridDateColDef';

export const getExcelJs = async () => {
  const excelJsModule = await Promise.resolve().then(() => exceljs);
  return excelJsModule.default ?? excelJsModule;
};
export const warnInvalidFormattedValue = buildWarning(['MUI X: When the value of a field is an object or a `renderCell` is provided, the Excel export might not display the value correctly.', 'You can provide a `valueFormatter` with a string representation to be used.']);
export const getFormattedValueOptions = (colDef, row, valueOptions, api) => {
