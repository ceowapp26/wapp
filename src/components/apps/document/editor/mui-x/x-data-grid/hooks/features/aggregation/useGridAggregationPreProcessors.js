...
import * as _wrapColumnWithAggregation from "./wrapColumnWithAggregation";
...
export const useGridAggregationPreProcessors = (apiRef, props) => {
  ...
  const updateAggregatedColumns = React.useCallback(columnsState => {
    ...
    columnsState.orderedFields.forEach(field => {
      ...
      if (haveAggregationColumnValue) {
        column = _wrapColumnWithAggregation.unwrapColumnFromAggregation({
          column
        });
      }
      if (shouldHaveAggregationValue) {
        column = _wrapColumnWithAggregation.wrapColumnWithAggregationValue({
          column,
  ...
  const addGroupFooterRows = React.useCallback(value => {
    ...
    return _gridAggregationUtils.addFooterRows({
      apiRef,
  ...
  const addColumnMenuButtons = React.useCallback((columnMenuItems, colDef) => {
    ...
    const availableAggregationFunctions = _gridAggregationUtils.getAvailableAggregationFunctions({
      aggregationFunctions: props.aggregationFunctions,
      colDef
    });
  ...
  const stateExportPreProcessing = React.useCallback(prevState => {
    ...
    const aggregationModelToExport = _gridAggregationSelectors.gridAggregationModelSelector(apiRef);
  ...
  const stateRestorePreProcessing = React.useCallback((params, context) => {
    ...
    if (aggregationModel != null) {
      apiRef.current.setState(_gridAggregationUtils.mergeStateWithAggregationModel(aggregationModel));
    }
    return params;
  }, [apiRef, props.disableAggregation]);
