  ...
  for (let depth = 0; depth < path.length; depth += 1) {
    ...
    if (depth === path.length - 1) {
      ...
      if (existingNodeIdWithPartialPath == null) {
        ...
        _utils.insertNodeInTree(leafNode, tree, treeDepths, previousTree);
      } else {
        ...
        if (existingNodeWithPartialPath.type === 'group' && existingNodeWithPartialPath.isAutoGenerated) {
          ...
          updatedGroupsManager?.addAction(parentNodeId, 'insertChildren');
          _utils.updateGroupNodeIdAndAutoGenerated({
            tree,
            previousTree,
            treeDepths,
    ...
    // We create an auto-generated group node.
      ...
      const nodeId = _utils.getGroupRowIdFromPath(path.slice(0, depth + 1));
      ...
      updatedGroupsManager?.addAction(parentNodeId, 'insertChildren');
      _utils.insertNodeInTree(_utils.updateGroupDefaultExpansion(autoGeneratedGroupNode, defaultGroupingExpansionDepth, isGroupExpandedByDefault), tree, treeDepths, previousTree);
      parentNodeId = nodeId;
    }
    // For the intermediary steps of the path
    // If a node matches the partial path, we use it as parent for the next step
      ...
      if (currentGroupNode.type !== 'group') {
        ...
        tree[existingNodeIdWithPartialPath] = _utils.updateGroupDefaultExpansion(groupNode, defaultGroupingExpansionDepth, isGroupExpandedByDefault);
      }
