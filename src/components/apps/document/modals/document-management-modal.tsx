...
const DocumentManagementModal = () => {
  const { user } = useUser();
  const { selectedDocument } = useGeneralContext();
  ...
  const { isOpen, onClose } = useDocumentManagement();
  const [roles, setRoles] = useState([]);
  const [permissions, setPermissions] = useState([]);
  const [emails, setEmails] = useState([]);
  const [memberEmails, setMemberEmails] = useState([]);
    ...
    const fetchDocument = async () => {
      if (selectedDocument) {
        const document = await getDocumentByID(
          selectedOrg
            ? { documentId: selectedDocument, activeOrgId: selectedOrg.orgId }
            : { documentId: selectedDocument }
        );
        _setDocument(document);
        if (document && document.metaData && document.metaData.orgs) {
          if (selectedOrg && selectedOrg.orgId && document.metaData.orgs.includes(selectedOrg.orgId)) {
            toast.warning("The organization already exists. Would you like to update the current status instead?");
            _setEvent("update");
          }
          const data = document.metaData.orgs.map((org) => ({
            orgId: org.orgId,
  ...
  }, [selectedDocument, selectedOrg, userMemberships]);
  ...
  const updateDocumentAndTable = async (newRecord) => {
    try {
      const updatedMetaData = { ..._document.metaData };
      ...
      toast.success('Document and table updated successfully!');
    } catch (error) {
      console.error('Error updating document and table:', error);
      toast.error('Failed to update document and table. Please try again.');
      throw error; 
    }
  };
  ...
  const handleAddOrUpdate = async () => {
    try {
      ...
      await updateDocumentAndTable(newRecord);
          ...
          <>
            {_document ? (
              <>
                ...
                <ModalBody>
                  <h3>Your current document: <span>{_document.title}</span></h3>
                  <SelectorSection>
                  ...
                  <TableResult 
                    data={tableData} 
                    onUpdateTable={updateDocumentAndTable}
              ...
              >
                ...
                <h2 className="text-xl font-semibold text-white mb-2">No Document Available</h2>
                <p className="text-gray-400 text-center max-w-sm">
                  It looks like there are no documents to display at the moment. 
                  Please reselect the document to continue.
                </p>
              </motion.div>
            )}
          </>
...
DocumentManagementModal.displayName = "DocumentManagementModal";
