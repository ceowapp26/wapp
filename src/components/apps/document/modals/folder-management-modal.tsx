  ...
  <Popover placement="bottom">
    ...
    <PopoverContent>
      <div className="px-1 py-2">
        <pre className="whitespace-pre-wrap overflow-auto max-h-96 text-gray-900/75 dark:text-white/75">
          {JSON.stringify(processModelData(item), null, 2)}
        </pre>
      </div>
    </PopoverContent>
...
export const TableResult = ({ data, onUpdateTable, onUpdateCell, onDeleteRow }) => {
  ...
  const getFolderByID = useMutation(api.chats.getFolderByID);
...
const FolderManagmentModal = () => {
  const { user } = useUser();
  const { selectedFolder } = useGeneralContext();
  const [_folder, _setfolder] = useState(null);
  ...
  const { isOpen, onClose } = useFolderManagement();
  const [roles, setRoles] = useState([]);
  const [permissions, setPermissions] = useState([]);
  const [emails, setEmails] = useState([]);
  const [memberEmails, setMemberEmails] = useState([]);
    ...
    const fetchFolder = async () => {
      if (selectedFolder) {
        const folder = await getFolderByID(
          selectedOrg
            ? { id: selectedFolder, activeOrgId: selectedOrg.orgId }
            : { id: selectedFolder }
        ).then(result => result.folder);;
        _setfolder(folder);
        if (folder && folder.metaData && folder.metaData.orgs) {
          if (selectedOrg && selectedOrg.orgId && folder.metaData.orgs.includes(selectedOrg.orgId)) {
            toast.warning("The organization already exists. Would you like to update the current status instead?");
            _setEvent("update");
          }
          const data = folder?.metaData?.orgs?.map((org) => ({
            orgId: org.orgId,
  ...
  }, [selectedFolder, selectedOrg, userMemberships]);
  ...
  const updatefolderAndTable = async (newRecord) => {
    try {
      const updatedMetaData = { ..._folder.metaData };
      ...
      toast.success('folder and table updated successfully!');
    } catch (error) {
      console.error('Error updating folder and table:', error);
      toast.error('Failed to update folder and table. Please try again.');
      throw error; 
    }
  };
  ...
  const handleAddOrUpdate = async () => {
    try {
      ...
      await updatefolderAndTable(newRecord);
          ...
          <>
            {_folder ? (
              <>
                ...
                <ModalBody>
                  <h3>Your current folder: <span>{_folder.folderName}</span></h3>
                  <SelectorSection>
                  ...
                  <TableResult 
                    data={tableData} 
                    onUpdateTable={updatefolderAndTable}
              ...
              >
                ...
                <h2 className="text-xl font-semibold text-white mb-2">No Folder Available</h2>
                <p className="text-gray-400 text-center max-w-sm">
                  It looks like there are no folders to display at the moment. 
                  Please reselect the folder to continue.
                </p>
              </motion.div>
            )}
          </>
...
FolderManagmentModal.displayName = "FolderManagmentModal";

export default FolderManagmentModal;
