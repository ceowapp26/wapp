        ...
        if (saveRef?.current) {
          toast.promise(
            htmlToImg(saveRef.current).then(imgData => {
              const title = usePortalStore.getState().chats?.[usePortalStore.getState().currentChatIndex].title.trim() ?? 'download';
        ...
        if (chats && saveRef?.current) {
          toast.promise(
            Promise.resolve().then(() => {
              const markdown = chatToMarkdown(chats[usePortalStore.getState().currentChatIndex]);
              const title = chats[usePortalStore.getState().currentChatIndex].title.trim() ?? 'download';
    ...
    },
    {
      ...
      onClick: async () => {
        ...
        if (chats) {
          toast.promise(
            Promise.resolve().then(() => {
              const chat = chats[usePortalStore.getState().currentChatIndex];
