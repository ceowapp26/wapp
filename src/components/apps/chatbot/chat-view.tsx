...
const ChatView = ({
  ...
  const { chatSession, setChatSession } = usePortalContext();
  ...
  useEffect(() => {
    if (isInsertedContent && insertedContent !== '' && insertedContent.trim() !== '' && !usePortalStore.getState().generating) {
      _setContent(insertedContent)
      handleGenerate();
    }
  }, [isInsertedContent, insertedContent]);
  ...
  const { handleAIDynamicFunc, handleStopChat } = useDynamicSubmit();
  ...
  const handleResetInsertContent = () => {
    setIsInsertedContent(false);
    setInsertedContent("");
  };

  const updateChatWithSession = () => {
    if (!chatSession || currentChatIndex === undefined) return;
    const updatedChats: ChatInterface[] = JSON.parse(
      JSON.stringify(usePortalStore.getState().chats)
    );
    updatedChats[currentChatIndex].messages = chatSession.getMessages();
  ...
  const handleSetup = useCallback(() => {
    ...
    if (isCode) setCodeGenerator(true);
    setCurrentCodeFile(null);
    setCurrentComponent(null);
  }, [setAiContext, setChatContext, setInputType, setOutputType, setIsSystemModel, chatModel, setIsWarningModalOpen, chatSession]);
  ...
  const handleGenerate = useCallback(async() => {
    ...
    if (!chatSession?.isWarning()) {
      const result = await handleAIDynamicFunc();
      if (result) {
        updateChatWithSession();
      }
      if (isInsertedContent) {
        handleResetInsertContent();
      }
    }
  }, [
    _content,
  ...
  const endContent = useMemo(() => (
    <div className="flex gap-4 justify-center items-center px-2 py-2 h-full">
      <GenerateButton isGenerating={usePortalStore.getState().generating} stop={handleStopChat} generate={handleGenerate} />
      {isVertical ? (
        <Popover>
          <PopoverTrigger>
            <Button
