...
const Menu = () => {
  ...
  const setHideSideMenu = usePortalStore((state) => state.setHideSideMenu);
  const {
    isLeftSidebarOpened,
  ...
  const handleSidebar = () => {
    if (routeType) {
      dispatch(setPortalContext(routeType));
    }
    if (!isLeftSidebarOpened) setIsLeftSidebarOpened(true);
  };

  return (
    <div className='flex relative top-14 h-full min-h-0 px-2 flex-col'>
      <div>
        <Button
