...
const ChatHistoryList = ({ filterKey, filterValue }: { filterKey?: string, filterValue?: string }) => {
  ...
  const archivedChats = usePortalStore((state) => state.archivedChats);
  const folders = usePortalStore((state) => state.folders);
  const archivedFolders = usePortalStore((state) => state.archivedFolders);
  ...
  const chatTitles = usePortalStore(
  ...
  const chatsRef = useRef<ChatInterface[]>(usePortalStore.getState().chats || []);
  const archivedChatsRef = useRef<ArchivedChatInterface[]>(usePortalStore.getState().archivedChats || []);
  const foldersRef = useRef<FolderCollectionInterface>(usePortalStore.getState().folders);
  const archivedFoldersRef = useRef<ArchivedFolderCollectionInterface>(usePortalStore.getState().archivedFolders);
  ...
  const updateArchivedFolders = useRef(() => {
    ...
    const _archivedChats = usePortalStore.getState().archivedChats;
    const _archivedFolders = usePortalStore.getState().archivedFolders;
  ...
  const updateFolders = useRef(() => {
    ...
    const _activeChats = usePortalStore.getState().chats;
    const _activeFolders = usePortalStore.getState().folders;
    ...
    if (_activeChats) {
      _activeChats.forEach((chat, index) => {
        ...
        if (
          ...
          index !== usePortalStore.getState().currentChatIndex
        )
          return;
    ...
    if (
      ...
      if (chats) {
        ...
        const updatedFolders: FolderCollectionInterface = JSON.parse(
            JSON.stringify(usePortalStore.getState().folders)
        );
