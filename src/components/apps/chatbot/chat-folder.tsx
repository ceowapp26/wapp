...
const ChatFolder = ({
  ...
  const isArchived = usePortalStore((state) => state.archivedFolders[folderId]?.isArchived);
  const folderName = isArchived ? usePortalStore((state) => state.archivedFolders[folderId]?.folder.folderName) : usePortalStore((state) => state.folders[folderId]?.folderName);
  const isExpanded = isArchived ? usePortalStore((state) => state.archivedFolders[folderId]?.folder.expanded) : usePortalStore((state) => state.folders[folderId]?.expanded);
  const color = usePortalStore((state) => state.folders[folderId]?.color);
  ...
  const handleRemoveFolder = () => {
    const archivedChats = JSON.parse(
      JSON.stringify(usePortalStore.getState().archivedChats)
    );
    const archivedFolders = JSON.parse(
      JSON.stringify(usePortalStore.getState().archivedFolders)
    );
            ...
            !isArchived ? (
              <React.Fragment>
                ...
                <button
                  ...
                  disabled={folderId === Object.keys(usePortalStore.getState().folders)[0]}
                >
                ...
                <button
                  ...
                  disabled={folderId === Object.keys(usePortalStore.getState().folders)[Object.keys(usePortalStore.getState().folders).length - 1]}
                >
