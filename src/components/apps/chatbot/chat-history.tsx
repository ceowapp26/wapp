...
const ChatHistory = React.memo(
  ({ chatId, documentId, userId, orgId, cloudChatId, chatTitle, chatIndex, isArchived }: { chatId: string, documentId: string, userId: string, orgId: string, cloudChatId: string, chatTitle: string, chatIndex: number, isArchived: boolean }) => {
    ...
    const active = usePortalStore((state) => state.currentChatIndex === chatIndex);
    ...
    const routeType = useMemo(() => {
      if (/\/myspace\/apps\/portal\/code/.test(currentPath)) {
        return 'code';
      } else if (/\/myspace\/apps\/portal\/document/.test(currentPath)) {
        return 'document';
      }
      return null;
    }, [currentPath]);
    ...
    const handleSidebar = () => {
      if (rightSidebarType !== 'right-ai' && routeType !== "code") setRightSidebarType('right-ai');
      if (!isRightSidebarOpened && routeType !== "code") setIsRightSidebarOpened(true);
    };
