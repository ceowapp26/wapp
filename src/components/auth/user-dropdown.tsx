    ...
    <DropdownMenu onOpenChange={setIsOpen}>
      ...
      >
          ...
          <DropdownMenuItem key={item.label} asChild>
            <Link
              className={`flex items-center p-2 rounded-md hover:bg-primary/5 transition-colors duration-200 ${
                index === 2 ? "text-primary" : index === DropdownItems.length - 1 ? "text-blue-500" : "text-foreground"
              }`}
              href={item.href}
            >
        ...
        <DropdownMenuItem asChild>
          <SignOutButton className="flex items-center w-full p-2 rounded-md hover:bg-red-100 transition-colors duration-200 text-red-500">
            <button>
