...
const SnippetItem = React.memo(
  ({ snippetId, cloudSnippetId, content, snippetIndex }: { snippetId: string, cloudSnippetId: Id<"snippets">, content: string, snippetIndex: number }) => {
  ...
  const editTitle = () => {
    ...
    handleUpdateCloudSnippet(cloudSnippetId, snippetIndex, updatedSnippets[snippetIndex]);
    setIsEdit(false);
  };
  ...
  const onDragStart = (event: React.DragEvent<HTMLDivElement>, snippetId: string, content: string) => {
   ...
   const onDrop = (event: React.DragEvent<HTMLDivElement>, targetId: string) => {
    ...
    if (draggedSnippetId) {
      const updatedSnippets = [...useDocumentStore.getState().snippets];
      const draggedIndex = updatedSnippets.findIndex(snippet => snippet.snippetId === draggedSnippetId);
      const targetIndex = updatedSnippets.findIndex(snippet => snippet.snippetId === targetId);
      if (draggedIndex !== -1 && targetIndex !== -1) {
        const [draggedSnippet] = updatedSnippets.splice(draggedIndex, 1);
        updatedSnippets.splice(targetIndex, 0, draggedSnippet);
        setSnippets(updatedSnippets);
        updatedSnippets.forEach((snippet, index) => {
          handleUpdateCloudSnippet(snippet.cloudSnippetId, index, snippet);
        });
      }
