...
export async function POST(req: Request): Promise<Response> {
  try {
    ...
    let { prompt, command, model, config } = await req.json();

    // Uncomment this if you want to check location support
      ...
      .with("DEBUG", () => [
        {
          role: "system",
          content: `
            Please debug this code and respond with the expected format as follows:
            {
              "line content": "error message"
            }
            Only include lines with errors. If there are no errors, return an empty object.
            The "line content" should be the exact content of the line with the error.
          `,
        },
        {
          role: "user",
