...
function getWorker() {
  if (workerPool.length < MAX_WORKERS) {
    const worker = new Worker(path.join(process.cwd(), 'workers', 'integration-test-runner.js'));
...
export async function POST(req: NextRequest) {
  try {
    const { userId, projectStructure } = await req.json();
    ...
    const result = await new Promise((resolve, reject) => {
      ...
      worker.postMessage({ userId, projectStructure });
    });
